...
context("Ratchet", () => {
    ...
    const message = new Uint8Array([1, 2, 3, 4, 5, 6, 7, 8, 9, 0]).buffer as ArrayBuffer;
    const encMessage: ArrayBuffer[] = [];

    before(async () => {
        const rnd = Secret.randomBytes(32);
        rootKey = await Secret.importHMAC(rnd);
    });

    it("SendingRatchet", async () => {
    ...
    context("ReceivingRatchet", () => {
        ...
        it("get message with custom counter", async () => {
            const sender = new ReceivingRatchet(rootKey);
            assert.equal(sender.counter, 0);
    ...
    context("asym ratchet", () => {
        ...
        context("create", () => {
            ...
            it("form PreKeyBundle and PreKeyMessage", async () => {
                const AliceID = await createIdentity(1);
                const BobID = await createIdentity(2);
